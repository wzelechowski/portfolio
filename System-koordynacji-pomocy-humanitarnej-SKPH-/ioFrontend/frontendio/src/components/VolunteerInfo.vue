    ...
    <div class="card h-100" style="width: 100%; padding: 20px;">
      <div class="card-body">
        ...
        <div class="container mt-5">
          ...
          <div  class="mb-3">
            <label class="form-label">{{  $t('profile-grade') }}</label>
            <input
                type="text"
                class="form-control"
                v-model="grade"
  ...
  methods: {
    async fetchVolunteers() {
      ...
      try {
        ...
        this.fetchGradesForAll(response.data.username)
        console.log(this.volunteers(0));
        this.loading = false;
      } catch (err) {
    ...
    async fetchGradesForAll(username) {
      try {
          const response = await axios.get('http://localhost:8080/api/test/calculateGrade', {
        ...
        this.error = error.message || $t('organization-error4');
      }
    },
    goBack() {
      this.$router.go(-1);
