...
class FrontendSupabaseUser extends BaseAuthUser {
  FrontendSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> frontendSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FrontendSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
