...
void main() async {
  ...
  testWidgets('AppBarTest', (WidgetTester tester) async {
    _overrideOnError();
    ...
    await tester.pumpAndSettle(); 
    print('Widoczne teksty: ${find.byType(Text).evaluate().map((e) => e.widget).whereType<Text>().map((t) => t.data).toList()}');
    expect(find.text('Manage the care time of your plants'), findsOneWidget);
    await tester.tap(find.text('List'));
    await tester.pumpAndSettle(); 
    print('Widoczne teksty: ${find.byType(Text).evaluate().map((e) => e.widget).whereType<Text>().map((t) => t.data).toList()}');
    expect(find.text('Shopping list for taking good care of your plants'), findsOneWidget);
    await tester.tap(find.text('Guide'));
    await tester.pump(const Duration(seconds: 4)); 
    await tester.pump(const Duration(seconds: 4)); 
    print('Widoczne teksty: ${find.byType(Text).evaluate().map((e) => e.widget).whereType<Text>().map((t) => t.data).toList()}');
    expect(find.text('Start looking for the species'), findsOneWidget);
  });
}
